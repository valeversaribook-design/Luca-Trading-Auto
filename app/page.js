...
export default function LucaTradingAuto() {
  ...
  // Giorni selezionabili dal CSV: da/a. Se scegli lo stesso giorno, usa solo quel giorno.
  const [dayFrom, setDayFrom] = useState("");
  const [dayTo, setDayTo] = useState("");

  // 3 scenari facoltativi: puoi compilare solo quelli che vuoi.
  // Ogni scenario usa un prezzo apertura e un prezzo chiusura personalizzati.
  ...
  const dayOptions = useMemo(() => {
    ...
    candles.forEach(c => {
      ...
      if (!map.has(k)) map.set(k, { key: k, label: dayLabel(c.time) });
    });
    return Array.from(map.values()).sort((a,b) => a.key.localeCompare(b.key));
  }, [candles]);

  const selectedDayFrom = dayFrom || dayOptions[0]?.key || "";
  const selectedDayTo = dayTo || dayOptions.at(-1)?.key || "";

  const candlesBySelectedDays = useMemo(() => {
    if (!dayOptions.length) return candles;
    const from = selectedDayFrom <= selectedDayTo ? selectedDayFrom : selectedDayTo;
    const to = selectedDayFrom <= selectedDayTo ? selectedDayTo : selectedDayFrom;
    return candles.filter(c => {
      const k = dayKey(c.time);
      return k >= from && k <= to;
    });
  }, [candles, dayOptions, selectedDayFrom, selectedDayTo]);

  useEffect(() => {
    if (!candlesBySelectedDays.length) return;
    setEntryIndex(0);
    setExitIndex(Math.min(1, candlesBySelectedDays.length - 1));
    setEntryPrice(candlesBySelectedDays[0]?.open.toFixed(3) || "");
    setExitPrice(candlesBySelectedDays[Math.min(1, candlesBySelectedDays.length - 1)]?.close.toFixed(3) || "");
  }, [selectedDayFrom, selectedDayTo, candlesBySelectedDays.length]);
  const activeCandle = preview || selected;
  const entry = candlesBySelectedDays[entryIndex];
  const exit = candlesBySelectedDays[exitIndex];
  ...
  function loadCSV(file) {
    Papa.parse(file, {
      ...
      complete: res => {
        ...
        const uniqueDays = Array.from(new Set(parsed.map(c => dayKey(c.time)))).sort();
        const firstDay = uniqueDays[0] || "";
        const lastDay = uniqueDays.at(-1) || "";
        setDayFrom(firstDay);
        setDayTo(lastDay);
  ...
  function setCandleAsEntry(c) {
    if (!c) return;
    const idx = candlesBySelectedDays.findIndex(x => x.id === c.id);
    if (idx >= 0) { setEntryIndex(idx); setEntryPrice(candlesBySelectedDays[idx].open.toFixed(3)); }
  }

  function setCandleAsExit(c) {
    if (!c) return;
    const idx = candlesBySelectedDays.findIndex(x => x.id === c.id);
    if (idx >= 0) { setExitIndex(idx); setExitPrice(candlesBySelectedDays[idx].close.toFixed(3)); }
  }
  ...
  function filteredCandles() {
    ...
    const base = candlesBySelectedDays.length ? candlesBySelectedDays : candles;
    ...
    for (let tries = 0; tries < 900; tries++) {
      ...
      if (forcedScenario || scenarios.length) {
        ...
        if (wantPositive) {
          ...
          direction = exitP >= entryP ? "sell" : "buy";
        }
      } else {
        // Modalità libera: usa valori reali OHLC della candela.
        const v1 = [c1.open, c1.high, c1.low, c1.close];
        const v2 = [c2.open, c2.high, c2.low, c2.close];

        entryP = v1[randInt(0, v1.length - 1)];
        exitP = v2[randInt(0, v2.length - 1)];
    ...
    <main className="page">
      ...
      <div className="layout">
        <aside className="side">
          ...
          <label>Giorno da</label><select value={selectedDayFrom} onChange={e=>setDayFrom(e.target.value)}>{dayOptions.map(d=><option key={d.key} value={d.key}>{d.label}</option>)}</select>
          <label>Giorno a</label><select value={selectedDayTo} onChange={e=>setDayTo(e.target.value)}>{dayOptions.map(d=><option key={d.key} value={d.key}>{d.label}</option>)}</select>
          ...
          <h2>Scenari apertura/chiusura opzionali</h2>
          <label>Scenario 1 apertura</label><input type="number" step="0.001" value={scenario1Open} onChange={e=>setScenario1Open(e.target.value)} placeholder="libero"/>
          <label>Scenario 1 chiusura</label><input type="number" step="0.001" value={scenario1Close} onChange={e=>setScenario1Close(e.target.value)} placeholder="libero"/>
          <label>Scenario 2 apertura</label><input type="number" step="0.001" value={scenario2Open} onChange={e=>setScenario2Open(e.target.value)} placeholder="libero"/>
          <label>Scenario 2 chiusura</label><input type="number" step="0.001" value={scenario2Close} onChange={e=>setScenario2Close(e.target.value)} placeholder="libero"/>
          <label>Scenario 3 apertura</label><input type="number" step="0.001" value={scenario3Open} onChange={e=>setScenario3Open(e.target.value)} placeholder="libero"/>
          <label>Scenario 3 chiusura</label><input type="number" step="0.001" value={scenario3Close} onChange={e=>setScenario3Close(e.target.value)} placeholder="libero"/>
        ...
        <section className="content">
          <div className="cards"><div><span>Candele selezionate</span><b>{candlesBySelectedDays.length}</b></div><div><span>Prima</span><b>{candles[0]?itDate(candles[0].time):"-"}</b></div><div><span>Ultima</span><b>{candles.at(-1)?itDate(candles.at(-1).time):"-"}</b></div><div><span>Profitto attuale</span><b className={totalProfit>=0?"pos":"neg"}>{money(totalProfit)}</b></div></div>
          <Chart candles={candlesBySelectedDays} trades={trades} selected={selected} preview={preview} onPick={setSelected} onPreview={setPreview}/>
