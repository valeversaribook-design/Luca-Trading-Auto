...
function candleMenu(c) {
  return `${itDate(c.time, false)} | O ${c.open.toFixed(2)} H ${c.high.toFixed(2)} L ${c.low.toFixed(2)} C ${c.close.toFixed(2)}`;
}
  ...
  return (
    <svg className="chart" ref={ref} viewBox={`0 0 ${W} ${H}`} onClick={e => onPick(candleFromEvent(e))} onMouseMove={e => onPreview(candleFromEvent(e))} onMouseLeave={() => onPreview(null)}>
      ...
      {[0,1,2,3,4].map(i => {
        ...
        return <g key={i}><line x1={L} x2={W-R} y1={yy} y2={yy} stroke="#1f2f43" /><text x={W-R-2} y={yy-5} fill="#8ea6bf" fontSize="12" textAnchor="end">{val.toFixed(2)}</text></g>;
      })}
      {visible.map((c, i) => {
        ...
        const isSelected = selected && selected.time.getTime() === c.time.getTime();
        const isPreview = preview && preview.time.getTime() === c.time.getTime();
        return <g key={i}>
      ...
      {trades.map((t, i) => {
        const oi = visible.findIndex(c => c.time.toISOString().slice(0,16) === t.openTime.toISOString().slice(0,16));
        const ci = visible.findIndex(c => c.time.toISOString().slice(0,16) === t.closeTime.toISOString().slice(0,16));
      ...
      {(preview || selected) && (() => {
        ...
        const idx = visible.findIndex(c => c.time.getTime() === mark.time.getTime());
        if (idx < 0) return null;
        const x = L + idx * step + step / 2;
        return <g>
          <line x1={x} x2={x} y1={T} y2={H-B} stroke="#60a5fa" strokeDasharray="5 5" />
          <rect x={Math.min(x + 12, W - 290)} y={42} width="270" height="132" rx="10" fill="#0f1c2e" stroke="#315d88" />
          <text x={Math.min(x + 28, W - 274)} y="68" fill="#ffffff" fontSize="14" fontWeight="700">{itDate(mark.time)}</text>
          <text x={Math.min(x + 28, W - 274)} y="94" fill="#cbd5e1" fontSize="13">Open: {mark.open.toFixed(2)}</text>
          <text x={Math.min(x + 28, W - 274)} y="116" fill="#cbd5e1" fontSize="13">High: {mark.high.toFixed(2)}</text>
          <text x={Math.min(x + 28, W - 274)} y="138" fill="#cbd5e1" fontSize="13">Low: {mark.low.toFixed(2)}</text>
          <text x={Math.min(x + 28, W - 274)} y="160" fill="#cbd5e1" fontSize="13">Close: {mark.close.toFixed(2)}</text>
        </g>;
      })()}
...
export default function LucaTradingAuto() {
  ...
  function loadCSV(file) {
    Papa.parse(file, {
      ...
      complete: res => {
        ...
        const parsed = rows.map(r => ({ time: parseDate(r[h.time]), open: toNum(r[h.open]), high: toNum(r[h.high]), low: toNum(r[h.low]), close: toNum(r[h.close]), volume: h.volume ? toNum(r[h.volume]) : 0 }))
          .filter(c => c.time && ![c.open,c.high,c.low,c.close].some(Number.isNaN))
          .sort((a,b) => a.time - b.time);
        setCandles(parsed); setTrades([]); setSelected(null); setPreview(null); setAutoSets([]);
        setEntryIndex(0); setExitIndex(Math.min(1, parsed.length - 1));
        setEntryPrice(parsed[0]?.open.toFixed(2) || "");
        setExitPrice(parsed[1]?.close.toFixed(2) || parsed[0]?.close.toFixed(2) || "");
      }
    });
  }
  ...
  function setCandleAsEntry(c) {
    if (!c) return;
    const idx = candles.findIndex(x => x.time.getTime() === c.time.getTime());
    if (idx >= 0) { setEntryIndex(idx); setEntryPrice(candles[idx].open.toFixed(2)); }
  }
  function setCandleAsExit(c) {
    if (!c) return;
    const idx = candles.findIndex(x => x.time.getTime() === c.time.getTime());
    if (idx >= 0) { setExitIndex(idx); setExitPrice(candles[idx].close.toFixed(2)); }
  }

  function addTrade() {
    if (!entry || !exit) return alert("Carica prima un CSV.");
    const ep = Number(entryPrice), xp = Number(exitPrice), l = Number(lot);
    if (Number.isNaN(ep) || ep < entry.low || ep > entry.high) return alert(`Prezzo apertura fuori dalla candela: ${entry.low.toFixed(2)} - ${entry.high.toFixed(2)}`);
    if (Number.isNaN(xp) || xp < exit.low || xp > exit.high) return alert(`Prezzo chiusura fuori dalla candela: ${exit.low.toFixed(2)} - ${exit.high.toFixed(2)}`);
    const ot = setSecond(entry.time, Number(entrySecond || 0));
    const ct = setSecond(exit.time, Number(exitSecond || 0));
    setTrades([...trades, { side, lot: l, openTime: ot, closeTime: ct, entry: ep, exit: xp, profit: pnl(side, ep, xp, l, Number(pointValue)) }]);
  }
  ...
  function makeTrade(wantPositive, pool) {
    for (let tries = 0; tries < 500; tries++) {
      ...
      if (wantPositive) {
        if (direction === "buy") {
          entry = rand(c1.low, c1.high);
          exit = rand(Math.max(c2.low, entry + 0.01), c2.high);
        } else {
          entry = rand(c1.low, c1.high);
          exit = rand(c2.low, Math.min(c2.high, entry - 0.01));
        }
      } else {
        if (direction === "buy") {
          entry = rand(c1.low, c1.high);
      ...
      entry = clamp(entry, c1.low, c1.high);
      exit = clamp(exit, c2.low, c2.high);
      const p = pnl(direction, entry, exit, l, Number(pointValue));
  ...
  async function downloadAutoZip() {
    ...
    for (const set of autoSets) {
      ...
      set.trades.forEach(t => {
        allRows.push(`${set.name},${t.side},${t.lot.toFixed(2)},${itDate(t.openTime)},${t.entry.toFixed(2)},${itDate(t.closeTime)},${t.exit.toFixed(2)},${t.profit.toFixed(2)}`);
      });
    }
    ...
    <main className="page">
      ...
      <div className="layout">
        ...
        <section className="content">
          <div className="cards"><div><span>Candele</span><b>{candles.length}</b></div><div><span>Prima</span><b>{candles[0]?itDate(candles[0].time):"-"}</b></div><div><span>Ultima</span><b>{candles.at(-1)?itDate(candles.at(-1).time):"-"}</b></div><div><span>Profitto attuale</span><b className={trades.reduce((a,t)=>a+t.profit,0)>=0?"pos":"neg"}>{money(trades.reduce((a,t)=>a+t.profit,0))}</b></div></div>
          <Chart candles={candles} trades={trades} selected={selected} preview={preview} onPick={setSelected} onPreview={setPreview}/>
          <div className="preview-panel"><div><h3>Anteprima candela</h3>{activeCandle ? <p><b>{itDate(activeCandle.time)}</b> — O {activeCandle.open.toFixed(2)} · H {activeCandle.high.toFixed(2)} · L {activeCandle.low.toFixed(2)} · C {activeCandle.close.toFixed(2)}</p> : <p>Passa sopra una candela nel grafico.</p>}</div><div className="mini-actions"><button onClick={()=>setCandleAsEntry(activeCandle)}>Usa come apertura</button><button onClick={()=>setCandleAsExit(activeCandle)}>Usa come chiusura</button></div></div>
