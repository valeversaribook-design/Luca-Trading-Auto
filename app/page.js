...
export default function LucaTradingAuto() {
  ...
  }, [dayOptions, selectedDayFrom, selectedDayTo]);
  ...
  function loadCSV(file) {
    Papa.parse(file, {
      ...
      complete: res => {
        ...
        const dayCandles = lastDay ? parsed.filter(c => dayKey(c.time) === lastDay) : parsed;
        setCandles(parsed);
        setDayFrom(firstDay);
        setDayTo(lastDay);
        setTrades([]);
  ...
  function addTrade() {
    ...
    if (Number.isNaN(ep)) return alert("Prezzo apertura non valido.");
    if (Number.isNaN(xp)) return alert("Prezzo chiusura non valido.");
  ...
  function makeTrade(wantPositive, pool, forcedScenario = null) {
    ...
    for (let tries = 0; tries < 900; tries++) {
      ...
      // Lotto a 3 decimali per evitare profitto sempre tondo.
      ...
      if (forcedScenario || scenarios.length) {
        ...
        // Modalità libera: valori precisi presi da OHLC della candela CSV.
  ...
  function generateAuto() {
    ...
    for (let s = 0; s < Number(screenCount); s++) {
      ...
      for (let attempt = 0; attempt < 500; attempt++) {
        ...
        for (const day of selectedDayKeys) {
          const dayPool = candles.filter(c => {
    ...
    if (!created.length) {
      alert("Non sono riuscito a creare screen con quei vincoli. Controlla giorni, orari, profitto min/max o range lotti.");
      return;
    }
    ...
    <main className="page">
      ...
      <div className="layout">
        <aside className="side">
          ...
          <label>Giorno da</label>
          <select value={selectedDayFrom} onChange={e=>{
          ...
          <label>Giorno a</label>
          <select value={selectedDayTo} onChange={e=>{
        ...
        <section className="content">
          <div className="cards"><div><span>Candele selezionate</span><b>{candlesForDay.length}</b></div><div><span>Prima</span><b>{candlesForDay[0]?itDate(candlesForDay[0].time):"-"}</b></div><div><span>Ultima</span><b>{candlesForDay.at(-1)?itDate(candlesForDay.at(-1).time):"-"}</b></div><div><span>Profitto attuale</span><b className={totalProfit>=0?"pos":"neg"}>{money(totalProfit)}</b></div></div>
